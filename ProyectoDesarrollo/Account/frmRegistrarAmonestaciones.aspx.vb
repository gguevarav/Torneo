...
Partial Public Class RegistrarAmonestaciones
    Inherits Page
    ...
    Protected Sub Page_Load()
        If (cbxPartidos.SelectedValue.ToString = "") Then
            ...
            If (Page.IsPostBack = False) Then
                ...
                Tabla.Columns.Add("Tipo amonestacion")
        ...
        End If
    End Sub
    Protected Sub RegistrarAmonestaciones()
        'Para insertar los datos debemos leer todas las filas de la tabla
        For i As Integer = 0 To grdListadoAmonestaciones.Rows.Count - 1
            ' Variables para uso en parámetros
            ' Tabla y campos a utilizarse
            Dim Tabla As String = "Amonestacion"
            Dim Campos As String = "idJugador, idPartido, idEquipo, TipoAmonestacion, minuto"
            ' Información del Equipo
            Dim ValoresInsertarAmonestacion As String
            ValoresInsertarAmonestacion = grdListadoAmonestaciones.Rows(i).Cells(1).Text.ToString & ", " & grdListadoAmonestaciones.Rows(i).Cells(3).Text.ToString & ", " & grdListadoAmonestaciones.Rows(i).Cells(5).Text.ToString & ", '" & grdListadoAmonestaciones.Rows(i).Cells(7).Text.ToString & "', '" & grdListadoAmonestaciones.Rows(i).Cells(8).Text.ToString & "'"
            Try
                Consulta.InsertarDatos(Tabla, Campos, ValoresInsertarAmonestacion)
            Catch ex As Exception
                Debug.Write(ex)
            End Try
        Next
    ...
    Protected Sub Agregar()
        ...
        Row("Tipo amonestacion") = cbxTipoAmonestacion.SelectedItem.ToString()
        Row("Minuto") = txtMinuto.Text
        Tabla.Rows.Add(Row)
        grdListadoAmonestaciones.DataSource = Tabla
        grdListadoAmonestaciones.DataBind()
        Session("Tabla") = Tabla
        Correlativo += 1
    End Sub
